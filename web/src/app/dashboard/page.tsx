...
const tierColor: Record<string, { dot: string; text: string; bg: string }> = {
  unacceptable: { dot: 'bg-gray-900', text: 'text-gray-900', bg: 'bg-gray-900/10' },
  high:         { dot: 'bg-red-500',  text: 'text-red-600',  bg: 'bg-red-50' },
  limited:      { dot: 'bg-orange-400', text: 'text-orange-600', bg: 'bg-orange-50' },
  minimal:      { dot: 'bg-emerald-500', text: 'text-emerald-600', bg: 'bg-emerald-50' },
...
function ScoreRing({ score, size = 120 }: { score: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      ...
      />
      <text x="50" y="46" textAnchor="middle" fontSize="18" fontWeight="700" fill="#111827">{score}</text>
      <text x="50" y="60" textAnchor="middle" fontSize="9" fill="#6b7280">out of 100</text>
    </svg>
  )
}
  ...
  return (
    <Shell>
      ...
      >
        ...
        {/* Gold glow */}
        <div
          className="absolute -bottom-16 -right-16 h-64 w-64 rounded-full opacity-15 pointer-events-none"
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            <div className="mb-1 flex items-center gap-2">
              ...
              <span className="text-[11px] font-medium text-white/50 uppercase tracking-widest">Live · AI Governance Platform</span>
            </div>
            <h1 className="mt-1 text-3xl font-bold tracking-tight text-white">Governance Overview</h1>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              ...
              <a
                href="/assessments"
                className="inline-flex items-center gap-1.5 rounded-xl border border-white/15 bg-white/8 px-4 py-2 text-sm font-medium text-white/80 transition hover:bg-white/15"
              >
          ...
          <div className="hidden lg:flex flex-col items-center gap-2 text-center">
            ...
            <div>
              <p className="text-xs font-semibold text-white/70">Compliance Score</p>
              <p className="text-[10px] text-white/35">avg across frameworks</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {statCards.map((c) => (
          <a key={c.label} href={c.href} className="card group flex flex-col gap-3 p-5 transition hover:shadow-md">
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <div className="card lg:col-span-2">
          <div className="card-header">
            ...
            <a href="/frameworks" className="ml-auto flex items-center gap-1 text-xs text-[#1B2A4A] hover:underline">
          ...
          <div className="p-5 space-y-5">
            ...
            {!loading && (stats?.framework_coverage ?? []).length === 0 && (
              <div className="flex flex-col items-center py-8 text-center">
                ...
                <p className="text-sm font-medium text-gray-500">No coverage data yet</p>
                <p className="mt-1 text-xs text-gray-400">Complete an assessment to see framework coverage.</p>
              </div>
            )}
          </div>
        </div>
                ...
                {(['unacceptable', 'high', 'limited', 'minimal', 'unknown'] as const).map((tier) => {
                  ...
                  return (
                    <div key={tier} className="flex items-center gap-2.5">
                      ...
                      <span className="w-20 text-xs capitalize text-gray-600">{tier}</span>
              ...
              <div className="mt-6 space-y-2 border-t border-gray-50 pt-4">
                {[
                  ...
                  <a
                    key={link.href}
                    href={link.href}
                    className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-xs font-medium text-gray-600 transition hover:bg-gray-50"
                  >
